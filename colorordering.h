...
{
public:
    inline virtual bool is_incomparable_not_equal(RGB &v, RGB &w) = 0;
    inline virtual bool islessequal(RGB &v, RGB &w) = 0;
    inline virtual bool isequal(RGB &v, RGB &w) = 0;
    inline virtual F32 getPriority(RGB &v) = 0;
};

class ColorMarginalOrdering : public ColorOrdering {
public:
    //incomparable and not equal
    inline bool is_incomparable_not_equal(RGB &v, RGB &w) {
        if (v.get_sum() <= w.get_sum()) {
    ...
    inline bool isequal(RGB &v, RGB &w) {
        if (v[0] != w[0] || v[1] != w[1] || v[2] != w[2]) return false;
        else return true;
    }

    inline F32 getPriority(RGB &v) {
        return -(v[0] + v[1] + v[2]);
    }
};
