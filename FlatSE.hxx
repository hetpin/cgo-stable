...
inline FlatSE &FlatSE::operator=(const FlatSE &src)
{
	if (this != &src)
	{
...
inline void FlatSE::setContext(const TSize *imSize)
{
	offsets.clear();
...
inline void FlatSE::setNegPosOffsets()
{
	for (int i = 0; i < 3; i++) {
		negativeOffsets[i] = 0;
		positiveOffsets[i] = 0;
...
inline Image<U8> FlatSE::toImage()
{
	TSize imageSize[3];
...
inline void FlatSE::makeSymmetric()
{
	FlatSE result;
...
inline void FlatSE::make2DEuclidianBall(int r)
{
	points.clear();
	offsets.clear();
...
inline void FlatSE::make3DAxialSegment(int l)
{
	points.clear();
	offsets.clear();
