...
public :
    struct Vertex {
        ...
        //By hetpin
        double get_sigx() {
            return this->pixels.size() * this->sum_color() / 3; //Try to normalize by number of channels
        }
        //By hetpin
        double get_sigx2() {
            // this->print_vertex();
            return this->pixels.size() * pow(this->sum_color() / 3, 2); //Try to normalize by number of channels
        }
        void print_vertex() {
            std::cout << "Vertex index=" << index << ", allNb=" << allNb.size() << ", pizels=" << pixels.size() << ", sigx=" << this->get_sigx() << endl;
...
public :
    ...
    int get_imsource_width() {
        return imSource.getSizeX();
    }
    int get_imsource_height() {
        return imSource.getSizeY();
    }
    FlatSE get_connexity(){
