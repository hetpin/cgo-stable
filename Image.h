...
public:
	...
	static int load_fits(image &img, Image <T> &im, int h, int w, int c);

	static int load_fits(std::vector<image> &imgs, Image <T> &im, int h, int w, int c);

	static int load_fits_quantize(std::vector<image> &imgs, Image <T> &im, int h, int w, int c, float &scale);
	...
	~Image() {
		if (this->data != 0) delete[] this->data; data = 0;
	}
	...
	bool isPosValid(Point <TCoord> p) const {
		return ((p.x >= 0) && (p.x < size[0]) && (p.y >= 0) && (p.y < size[1])
		        && (p.z >= 0) && (p.z < size[2]));
	};
